team1_num = 5
team2_num = 6
score_1 = 40
score_2 = 42
team1_time = 1552.512
team2_time = 2153.31451
tasks_total = 82
time_avg = 45.2
challenge_result1 = 'Победа команды Мастера кода!'
challenge_result2 = 'Победа команды Волшебники данных!'

print('В команде Мастера кода участникков: %s!'% team1_num)
print('Итого сегодня в командах участников: %s и %s!'% (team1_num, team2_num))
print('Команда Волшебники данных решила задач: {}!'.format(score_2))
print('Волшебники данных решили задачи за {}c)'.format(team2_time))
print(f'Команды решили {score_1} и {score_2} задач')
if score_2 > score_1 or score_1 == score_2 and team2_time > team1_time:
    print(f'Результат битвы: {challenge_result2}')
elif score_2 < score_1 or score_2 == score_1 and team2_time < team1_time:
    print(f'Результат битвы: {challenge_result1}')
if score_1 + score_2 and (team1_time + team2_time) / (score_1 + score_2):
    print(f'Сегодня было решено {score_1 + score_2} '
          f'задачи в среднем по {(team1_time + team2_time) / (score_1 + score_2)} секунды на задачу!.')
